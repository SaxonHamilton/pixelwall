...
function stringKeyToBytes(pk: string): Uint8Array | undefined {
  if (!pk) return undefined;
  try {
    if (pk.startsWith("0x")) return hexToBytes(pk);
    return base64ToBytes(pk);
  } catch {
    return undefined;
  }
}
...
export const createFhevmInstance = async (parameters: {
  ...
  let pub:
    | { publicKey: Uint8Array; publicParams: Uint8Array }
    | undefined = undefined;
  try {
    pub = await publicKeyStorageGet(aclAddress);
  } catch {
    // 缓存缺失时，先不带公钥创建实例，待实例创建成功后再缓存
  }
  throwIfAborted();
  ...
  const config: any = {
    ...
    // 让 SDK 自行拉取/校验公钥与参数；若本地缓存存在，也不主动传，避免类型不匹配
  };
  ...
  if (pk && pp) {
    const pkBytes = stringKeyToBytes(pk);
    if (pkBytes && pkBytes.length) {
      await publicKeyStorageSet(aclAddress, pkBytes, pp);
    }
  }
